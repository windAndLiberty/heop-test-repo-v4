/**
 * 端到端真实集成测试 V3：使用 Direct API（绕过 MCP stdio）
 * 测试目标：
 * 1. 初始化项目 → SSOT
...
const TEST_DIR = '/tmp/heop-real-test-v3';
const SSOT_DIR = '/tmp/heop-real-test-ssot-v3';
const PROJECT_ID = 'test-real-integration-v3';
const REPO_NAME = 'heop-test-repo-v3';

async function runTest() {
  console.log('=== HEOP Real Integration Test V3 (Direct API) ===\n');
  ...
  fs.writeFileSync(prdPath, `# Test Project V3
...
`);
  ...
  const bootstrapRes = await plugin.deepcodeBootstrap({
    project_id: PROJECT_ID,
    requirements_dir: TEST_DIR,
    constraints_json: constraintsPath,
    working_dir: TEST_DIR,
