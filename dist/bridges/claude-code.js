...
class ClaudeCodeBridge {
    ...
    async execute(args) {
        ...
        if (!allowedStates.includes(project.state)) {
            throw new Error(`Project state '${project.state}' does not allow incremental development. ` +
                `Expected one of: ${allowedStates.join(', ')}`);
        }
        // Create or update task record
        let actualTaskId = task_id;
        const existingTask = this.store.getTasks(project_id).find((t) => t.id === task_id);
        if (!existingTask) {
        ...
        try {
            ...
            const output = await this.spawnClaudeCode(project_id, goal, contextPackage, working_dir);
            // Parse output
            ...
            this.provenance.logProvenance(project_id, factId, 'CREATE', 'claude', `Claude Code task: ${goal}`, result.summary || result.error);
            return {
                content: [
                    {
                        type: 'text',
    ...
    async spawnClaudeCode(projectId, goal, contextPackage, workingDir) {
...
Requirements: ${(contextPackage.requirements || []).map((r) => r.text).join('; ')}
Decisions: ${(contextPackage.decisions || []).map((d) => `${d.context}=${d.choice}`).join('; ')}
`;
    ...
    async applyDiff(projectId, diff, workingDir) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                const child = (0, child_process_1.spawn)('git', ['apply', diffPath], { cwd });
                child.on('close', (code) => {
                    if (code === 0) {
                        resolve();
                    }
