...
exports.IssueAutomation = void 0;
class IssueAutomation {
    ...
    async createStructuredIssue(args) {
        ...
        try {
            ...
            const issueResult = await this.createIssue(title, body, labels);
            // Record in SSOT
            if (this.store) {
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            labels,
                            url: issueResult.url,
                            related_decisions: issueData.relatedDecisions.map((d) => d.id),
    ...
    assembleIssueData(projectId, taskId) {
        const project = this.store ? this.store.getProject(projectId) : null;
        const task = this.store
            ? this.store.getTasks(projectId).find((t) => t.id === taskId)
            : null;
    ...
    buildIssueBody(data) {
        ...
        if (allDecisions.length > 0) {
            const outdated = allDecisions.filter((d) => !relatedDecisions.find((rd) => rd.id === d.id));
    ...
    suggestNextSteps(currentState, task, milestones) {
        const steps = [];
        switch (currentState) {
            case 'CREATED':
            ...
            case 'INCREMENTAL_DEV':
                ...
                break;
            case 'TESTING':
                const unachieved = milestones.filter((m) => !m.achieved_at);
...
exports.IssueAutomation = IssueAutomation;
