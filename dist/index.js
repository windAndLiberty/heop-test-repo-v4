...
class HEOPPlugin {
    ...
    async adoptProject(args) {
        ...
        for (const decision of inferredDecisions) {
            this.store.addDecision(project_id, decision.context, decision.choice, decision.rationale, decision.confidence || 0.7, 'hermes-inferred');
        }
        ...
        this.provenance.logProvenance(project_id, factId, 'CREATE', 'human', `Adopted existing project from ${working_dir}`, `Tech stack: ${JSON.stringify(finalStack)}, Inferred decisions: ${inferredDecisions.length}`);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    detectTechStack(workingDir) {
        ...
        // Detect database
        if (fs.existsSync(path.join(workingDir, 'prisma')))
            stack.database = 'prisma';
        else if (fs.existsSync(path.join(workingDir, 'migrations')))
            stack.database = 'sqlx';
        else
            stack.database = 'unknown';
        return stack;
    }
    inferDecisions(workingDir, techStack) {
        const fs = require('fs');
        const path = require('path');
        const decisions = [];
