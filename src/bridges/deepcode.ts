...
export class DeepCodeBridge {
  ...
  async execute(args: DeepCodeInput): Promise<any> {
    return this.bootstrap(args);
  }

  async bootstrap(args: DeepCodeInput): Promise<any> {
    ...
    try {
      ...
      const output = await this.spawnBootstrapAgent(
        project_id,
        requirements_dir,
        constraints_json,
        working_dir
      ...
      for (const decision of decisions) {
        ...
        this.provenance.logProvenance(
          ...
          'CREATE',
          'deepcode',
          `DeepCode bootstrap for ${requirements_dir}`,
          decision.rationale
        );
      }
  ...
  private parseRequirements(requirementsDir: string): Array<{
    source_file: string;
    text: string;
    priority?: number;
  }> {
    const requirements: Array<{ source_file: string; text: string; priority?: number }> = [];
  ...
  private inferPriority(text: string): number {
    ...
    if (lower.includes('critical') || lower.includes('must') || lower.includes('essential')) return 1;
    if (lower.includes('high') || lower.includes('should')) return 2;
    if (lower.includes('medium') || lower.includes('could')) return 3;
    if (lower.includes('low') || lower.includes('optional') || lower.includes('nice')) return 5;
    return 5;
  }

  private async spawnBootstrapAgent(
    projectId: string,
  ...
  private async tryDeepCodeHku(
    ...
    return new Promise((resolve, reject) => {
      ...
      const child = spawn('deepcode', args, {
  ...
  private async spawnClaudeBootstrap(
    projectId: string,
    requirementsDir: string,
    constraintsJson?: string,
    workingDir?: string
...
Generate the project skeleton now.`;
  ...
  private generatePaperMd(requirementsDir: string, outputPath: string, constraintsJson?: string): void {
    const fs = require('fs');
...
`;
  ...
  private generateFallbackOutput(requirementsDir: string, constraintsJson?: string): string {
    ...
    let reqCount = 0;
    if (fs.existsSync(requirementsDir)) {
      reqCount = fs.readdirSync(requirementsDir).filter((f: string) => f.endsWith('.md')).length;
    }
  ...
  private extractDecisions(output: string, constraintsJson?: string): Array<{
    context: string;
    choice: string;
    rationale: string;
    confidence: number;
  ...
  private async initGit(projectId: string, workingDir?: string): Promise<void> {
    const cwd = workingDir || process.cwd();
    ...
    try {
      ...
      if (!fs.existsSync(gitDir)) {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          child.on('close', (code) => {
            if (code === 0) resolve();
            else reject(new Error(`git init failed: ${code}`));
          });
